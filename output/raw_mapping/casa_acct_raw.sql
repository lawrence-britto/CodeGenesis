...
FROM
(select * FROM c_acct where DATA_SRC = 'EBB' AND SOURCE_COUNTRY_CODE = 'VN' and process_id = 'EBB~VN~EBBS_VN_ACCOUNT~C_ACCT~1') cacct
LEFT JOIN (select *FROM C_PRTY_ID where DATA_SRC = 'EBB' AND SOURCE_COUNTRY_CODE = 'VN' and ID_TP_CD = 'N01') core_PID
on trim(cacct.prty_srogt_id) = trim(core_PID.PRTY_SROGT_ID)
and core_PID.prty_srogt_id <> "
LEFT JOIN (select prty_srogt_id, SEGMT_CD,PRTY_SEGMT_ID,PRTY_SEGMT_CLAS, ENTRP_PRTY_SROGT_ID,ENTRP_PRTY_REF FROM c_prty
where DATA_SRC = 'EBB' AND SOURCE_COUNTRY_CODE = 'VN') core_prty
on trim(cacct.prty_srogt_id) = trim(core_prty.prty_srogt_id)
and core_prty.prty_srogt_id <> "
LEFT JOIN (select *FROM C_ACCT_BAL_MTRC where
data_src = 'EBB' and source_country_code = 'VN') cabal
on cabal.acct_srogt_id = cacct.acct_srogt_id and cabal.acct_srogt_id <> "
LEFT JOIN (select acct_srogt_id,prty_srogt_id,PRIM_PRTY_IND FROM C_PRTY_ACCT_RLTD where data_src = 'EBB' and source_country_code = 'VN'
and process_id = 'EBB~VN~EBBS_VN_ACCOUNT~C_PRTY_ACCT_RLTD~1') ptyltd
on ptyltd.acct_srogt_id = cacct.acct_srogt_id and trim(ptyltd.prty_srogt_id) = trim(cacct.prty_srogt_id)
LEFT JOIN (select * FROM E_ACCT_BASE where data_src = 'EBB' and source_country_code = 'VN'
and process_id = 'EBB~VN~EBBS_VN_ACCOUNT~E_ACCT_BASE~1')
eacct on eacct.acct_srogt_id = cacct.acct_srogt_id
and eacct.acct_srogt_id <> "
LEFT JOIN (select prod_srogt_id, PROD_CAT_DESC, DOM_PROD_DESC, OD_IND
FROM C_PROD where data_src = 'EBB' and source_country_code = 'VN') cprod
